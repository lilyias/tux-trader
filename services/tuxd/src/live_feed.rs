...
/// Background sampler: keeps appending real SOLUSDT mids for the dashboard.
pub fn spawn_price_sampler(store: std::sync::Arc<tux_core::store::Store>, symbol: &'static str) {
        ...
        loop {
            ...
            match fetch_book(symbol).await {
                Ok(b) => {
                    ...
                    let _ = store.save_market_tick("SOL-USDT", b.bid, b.ask, mid, at);
                }
                Err(e) => {
...
pub async fn fetch_book(symbol: &str) -> anyhow::Result<LiveBook> {
    ...
    let client = reqwest::Client::builder()
        ...
        .build()?;
    let raw: BookTicker = client.get(&url).send().await?.error_for_status()?.json().await?;
...
pub async fn fetch_last_price(symbol: &str) -> anyhow::Result<Decimal> {
    ...
    let client = reqwest::Client::builder()
        ...
        .build()?;
    let raw: serde_json::Value = client.get(&url).send().await?.error_for_status()?.json().await?;
    raw.get("price")
        .and_then(|v| v.as_str())
        .and_then(dec)
